...
Maj<T, Bitmap>::Maj(std::shared_ptr<const std::vector<T>> initVec, double tau) 
    ...
    for(const T& element: elements) {
        ...
        for(int i = 0; i < int(n_); ++i) {
            for(int len = int(n_) - i; len >= 1;--len) {
    ...
    for(const auto& [element, vectorIntervals]: C) {
        ...
        for(int i = 0; i < int(n_);) {
            int j = i;
            while(j < int(n_) && ft.query(j + 1)) {
                j++;
            }
            if(j != i) {
    ...
    for(const auto& a: aVec) {
        A_.emplace_back(a);
        countMem_ += A_.back().memory();
    }

    for(const auto& m: mVec) {
        M_.emplace_back(m);
        countMem_ += M_.back().memory();
    }
...
template<typename T, typename Bitmap>
int Maj<T, Bitmap>::findFirstMajority(int i, int j, double tauPrime) const {
    // std::cout << " call with i,j = " << i << " " << j << std::endl;
    if(tauPrime < tau_) {
    ...
    for(size_t k = 0; k < A_.size(); ++k) {
        ...
        try {
            ...
            std::cout << "fail: iPrime, jPrime = " << iPrime << " " << jPrime << std::endl;
            return -1;
        }
        ...
        int rankI;
        try {
        rankJ = jPrime - 1 >= 0 ? m.rank(1, jPrime - 1) : 0;
        rankI = iPrime - 2 >= 0 ? m.rank(1, iPrime - 2) : 0;
        } catch(...) {
            std::cout << "falla 2: iPrime, jPrime = " <<  iPrime << " " << jPrime << std::endl;
            return -1;
        }
